...
class GitHubRepoManager {
  constructor() {
    ...
    this.npmDir = path.join(this.uiPlatformRoot, 'npm'); // Changed from tempDir to npmDir
    this.packagesDir = path.join(this.uiPlatformRoot, 'packages'); // Source packages
...
For detailed documentation, see: GITHUB_REPOSITORY_MANAGEMENT_GUIDE.md
  ...
  async createRepository(packageName, options = {}) {
    ...
    try {
      ...
      // Use safe git operation for adding remote (handles conflicts)
      this.safeGitOperation(`git remote add origin https://github.com/${owner}/${packageName}.git`);
      ...
      // Use safe git operation for push (handles conflicts automatically)
  ...
  async publishToNPM(packageName, options = {}) {
    ...
    try {
      ...
      console.log(`📁 Changing to directory: ${preparedDir}`);
      process.chdir(preparedDir);
      ...
      // ALWAYS return to original directory - this prevents navigation issues
  ...
  // Helper function for safe git operations with automatic conflict resolution
  safeGitOperation(command, options = {}) {
    const maxRetries = 3;

    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(`🔄 Git operation (attempt ${attempt}/${maxRetries}): ${command}`);
        return execSync(command, { stdio: 'inherit', ...options });
      } catch (error) {
        console.log(`⚠️  Git operation failed (attempt ${attempt}/${maxRetries}): ${error.message}`);

        // Check for specific git issues and try to resolve them
        if (error.message.includes('merge conflict') || error.message.includes('CONFLICT')) {
          ...
          try {
            ...
            if (status.includes('UU')) {
              console.log('📝 Found merge conflicts, resolving automatically...');
              // Accept local changes for conflicts
              execSync('git add .', { stdio: 'inherit' });
              execSync('git commit -m "Resolve merge conflicts automatically"', { stdio: 'inherit' });
              continue; // Retry the original command
            }
          } catch (resolveError) {
            console.log(`⚠️  Conflict resolution failed: ${resolveError.message}`);
          }
        }

        if (error.message.includes('remote already exists')) {
          console.log('🔧 Remote already exists, removing and re-adding...');
          try {
